...
function getStatus(indicator, value) {
    ...
    switch (indicator.ticker) {
        case "BAMLH0A0HYM2":
            if (value >= 6) return { text: "위험", className: "danger" };
            if (value >= 5) return { text: "경계", className: "warning" };
            if (value >= 4) return { text: "주의", className: "caution" };
            return { text: "안정", className: "safe" };

        case "BAMLH0A3HYC":
            if (value >= 10) return { text: "위험", className: "danger" };
            if (value >= 8) return { text: "경계", className: "warning" };
            if (value >= 6) return { text: "주의", className: "caution" };
            return { text: "안정", className: "safe" };

        case "VIXCLS":
            if (value >= 40) return { text: "위험", className: "danger" };
            if (value >= 30) return { text: "경계", className: "warning" };
            if (value >= 20) return { text: "주의", className: "caution" };
            return { text: "안정", className: "safe" };

        case "NFCI":
            if (value >= 1) return { text: "위험", className: "danger" };
            if (value >= 0.5) return { text: "경계", className: "warning" };
            if (value >= 0) return { text: "주의", className: "caution" };
            return { text: "완화", className: "safe" };

        case "T10Y2Y":
            if (value <= -1) return { text: "주의", className: "caution" };
            return { text: "정상", className: "safe" };

        case "SAHMREALTIME":
            if (value >= 0.5) return { text: "침체 신호", className: "danger" };
            return { text: "정상", className: "safe" };
...
function createCard(indicator, data) {
    ...
    return `
        <div class="card">
            ...
            <div class="value">
                ${formatValue(value, indicator.decimals, indicator.unit)}
            </div>
...
async function loadData() {
    try {
        ...
        const overall = calculateOverallRisk(allData);
        ...
        app.innerHTML = `
            <div class="overall ${overall.className}">
                ...
                <div class="overall-time">
                    데이터 업데이트:
                    ${json.updated_at
                        ? new Date(json.updated_at).toLocaleString("ko-KR")
                        : "--"}
                </div>
            </div>
